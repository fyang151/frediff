...
int diff(
  ...
  for (int D = 0; D <= MAX; D++) {
    // lemma 1: snakes after D edits must end up on diagonals k in {-D, -D+2, ... D-2, D}
    for (int k = -D; k <= D; k += 2) {
      int k_adj = k + MAX;
      ...
      // landing on a middle path in the forward snake means the optimal snake has odd length
      // vice versa for reverse snake
      if (delta % 2 == 1 
          // reverse snake will have only visited diagonals k - delta in [-(D-1), (D-1)]
          // we decrement from wavefront D because reverse snake is only at the previous iteration of D
    ...
    // reverse snake, very similar to forward snake, but we need to adjust for the reverse
    // to forward starting diagonal with delta
    for (int k = -D; k <= D; k += 2) {
      int k_adj = k + MAX;
